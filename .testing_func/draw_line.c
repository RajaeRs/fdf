...
/*   Updated: 2022/12/24 15:12:07 by rrasezin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    draw_line(int width, int length)
{
	...
	int yy = get_line_number("test_maps/42.fdf");
	int	xx = get_colomn_number("test_maps/42.fdf");
	float x_screen;
    float y_screen;
	...
	while (y < yy)
	{
		while (x < xx)
		{
        	...
        	x_screen = (point->x_map*15 - point->y_map*15)*cos(angle)+150;
        	y_screen = (point->x_map*15 + point->y_map*15)*sin(angle) - point->z_map+50;
			printf("x : %d |y : %d |z : %d |color : %d\n", point->x_map, point->y_map, point->z_map, point->color);
            printf("x_screen : %f|y_screen : %f\n", x_screen, y_screen);
			if (point->z_map == 10)
				my_mlx_pixel_put(&mlx_img, x_screen , y_screen , point->color);
			else 
