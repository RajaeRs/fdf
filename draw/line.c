...
/*   Updated: 2023/01/02 03:00:48 by rrasezin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	line(t_data *data, t_point *p0, t_point *p1)
{
	...
	l.dx = l.dx / l.step;
	l.dy = l.dy / l.step;
	l.x = p0->x_map + LENGTH / 2;
	l.y = p0->y_map + WIDTH / 10;
	l.i = 1;
	while (l.i <= l.step)
	{
...
// 	iso_p(p1);
// 	line (&data, p1, p0 );
// 	// mlx_pixel_put(ptr, win,
//p0->x_map, p0->y_map, p0->color); // drawing p0 after iso
// 	// mlx_pixel_put(ptr, win, p1->x_map,
//p1->y_map, p1->color); // // drawing p1 after iso
// 	// line (p1, p0, ptr, win);
