...
/*   Updated: 2023/01/02 10:04:26 by rrasezin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	mlx_creat(char *map, char *title)
{
	t_mlx	mlx;

	mlx.mlx_ptr = mlx_init();
	mlx.win = mlx_new_window(mlx.mlx_ptr, WIDTH, LENGTH, title);
	mlx.img.img = mlx_new_image(mlx.mlx_ptr, WIDTH, LENGTH);
...
int	main(int argc, char **argv)
{
	...
	i = 0;
	(void)argc;
	titel = (ft_strnstr(argv[1], "/", 10)) + 1;
	ti = malloc (strlen(titel) - 4);
	while (titel && titel[i] != '.')
	{
		ti[i] = titel[i];
		i++;
